...
class Model(object):
    ...
    def inference(self, x):
        ...
        with tf.variable_scope('BLSTM1') as scope:
            ...
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(
                ...
                sequence_length=[FRAMES_PER_SAMPLE] * self.batch_size, # [FRAMES_PER_SAMPLE,...]长度为batch_size
                dtype=tf.float32)
            state_concate = tf.concat(outputs, 2) # 沿着维度2进行concat，这写的应该有问题应该是tf.concat(outputs, 2)
    ...
    def loss(self, embeddings, Y, VAD):
        ...
        # tf.matmul(embeddings_v, tf.transpose(embeddings_v, [0,2,1])) (batch_size, FRAMES_PER_SAMPLE*NEFF, FRAMES_PER_SAMPLE*NEFF)
        # tf.matmul(Y_v, tf.transpose(Y_v, [0,2,1])) (batch_size, FRAMES_PER_SAMPLE*NEFF, FRAMES_PER_SAMPLE*NEFF)
        # loss_batch = tf.nn.l2_loss(
        #     tf.matmul(embeddings_v, tf.transpose(embeddings_v, [0,2,1])) - tf.matmul(Y_v, tf.transpose(Y_v, [0,2,1])))
